...
/* 
 * ===  FUNCTION  ======================================================================
 *         Name:  get_source_porfile
 *  Description:   get the profile token of the next profile

 * =====================================================================================
 */
static int get_next_profile_toke(sqlite3 *db, char *sql)
{
    char *zErrMsg = 0;
    int nrow = 0, ncolumn = 0;
    char **azResult; //二维数组存放结果
    sprintf(sql, "select value from media_config where chengji=1 and name='Token' order by value");
    sqlite3_get_table( db , sql , &azResult , &nrow , &ncolumn , &zErrMsg );
    int next_toke = atoi(azResult[1])+ 1;
}
...
int  __ns2__CreateProfile(struct soap* soap, struct _ns2__CreateProfile *ns2__CreateProfile, struct _ns2__CreateProfileResponse *ns2__CreateProfileResponse){
    ...
    if(ns2__CreateProfile != NULL){
        ...
        sqlite3_get_table( db , sql , &azResult , &nrow , &ncolumn , &zErrMsg );
        sprintf(sql, "insert into media_config (profile_name, name, value,  chengji) values ('%s', 'Name', '%s', 1)", ns2__CreateProfile->Name);
        sqlite3_get_table( db , sql , &azResult , &nrow , &ncolumn , &zErrMsg );
        memset(sql, 0, SQL_LENS);
        int profile_toke = get_next_profile_toke(db, sql);
        sprintf(string_toke, "%d", profile_toke);
        sprintf(sql, "insert into media_config (profile_name, name, value,  chengji) values ('%s', 'Token', '%s', 1)", ns2__CreateProfile->Name, string_toke);
    ...
    ns2__CreateProfileResponse->Profile = pProfile;

    pProfile->Name =  (char *)soap_malloc(soap,sizeof(ns2__CreateProfile->Name));
    memset( pProfile->Name, 0, sizeof(ns2__CreateProfile->Name));
    strcpy( pProfile->Name, ns2__CreateProfile->Name);

    pProfile->token =  (char *)soap_malloc(soap,strlen(string_toke));
    memset( pProfile->token, 0, strlen(string_toke));
    strcpy( pProfile->token, string_toke);       

    // get_source_porfile(soap, ns2__CreateProfile->Name, db, pProfile);
...
static int sql_atoi(sqlite3 *db, int *val, char *sql)
{
    char *ret = find_by_sqlstring(db, sql);
    if(ret != NULL)
    {
        *val = atoi(ret);
        return 1;
...
int get_source_porfile(struct soap* soap,char *profile_name, sqlite3 *db, struct ns3__Profile *pProfile)
{
    ...
    sql_atoi(db, &(pVideoSourcesConfig->UseCount), sql);
    ...
    sql_atoi(db, &(pVideoResolution->Width), sql);
    combin_str(sql, profile_name, 3, "height", "resolute"); 
    sql_atoi(db, &(pVideoResolution->Height), sql);
    ...
    sql_atoi(db, &(pVideoEncoderConfig->UseCount), sql);
    ...
    int EncodeType;
    int stat = sql_atoi(db, &(EncodeType), sql);
    if (stat)
    {
        switch(EncodeType){
            case 0: 
    ...
    char *quality_val = find_by_sqlstring(db, sql);
    if(quality_val != NULL)
    {
        pVideoEncoderConfig->Quality = atof(quality_val); 
    }
    ...
    sql_atoi(db, &(pVideoRateCtrl->FrameRateLimit), sql);
    combin_str(sql, profile_name, 3, "EncodingInterval", "RateControl");
    sql_atoi(db, &(pVideoRateCtrl->EncodingInterval), sql);
    combin_str(sql, profile_name, 3, "BitrateLimit", "RateControl");
    sql_atoi(db, &(pVideoRateCtrl->BitrateLimit), sql);
    ...
    if(stat){
        switch(EncodeType){
            ...
            case 2:		    
                ...
                sql_atoi(db, &(pH264Config->GovLength), sql);
                combin_str(sql, profile_name, 3, "H264Profile", "H264");

                int h264_profile, h264_stat;
                h264_stat = sql_atoi(db, &(h264_profile), sql);
                if(h264_stat)
                {
                    switch(h264_profile){
                        case 0:
    ...
    int type_val, type_stat;
    type_stat = sql_atoi(db, &(type_val), sql);
    if(type_stat){
        switch(type_val){
            case 0:
    ...
    sql_atoi(db, &(pMulticastConfig->Port), sql);
    combin_str(sql, profile_name, 3, "TTL", "MulticastConfig");
    sql_atoi(db, &(pMulticastConfig->TTL), sql);
    combin_str(sql, profile_name, 3, "AutoStart", "MulticastConfig");


    int auto_stat, auto_val;
    auto_stat = sql_atoi(db, &(pMulticastConfig->TTL), sql);
    if(auto_stat)
    {
        switch(auto_val){
            case 0:
...
int  __ns2__AddVideoEncoderConfiguration(struct soap* soap, struct _ns2__AddVideoEncoderConfiguration *ns2__AddVideoEncoderConfiguration, struct _ns2__AddVideoEncoderConfigurationResponse *ns2__AddVideoEncoderConfigurationResponse){
    ...
    char *sql = (char*)malloc(SQL_LENS);
    memset(sql, 0, SQL_LENS);
    sprintf(sql, "select profile_name from media_config where chengji=2 and name='Token' and value=%d", atoi(ns2__AddVideoEncoderConfiguration->ConfigurationToken));
    sqlite3_get_table( db , sql , &azResult , &nrow , &ncolumn , &zErrMsg );
    char *profile_name = azResult[1];
    
    memset(sql, 0, SQL_LENS);
    sprintf(sql, "select profile_name from media_config where chengji=1 and name='Token' and value=%d", atoi(ns2__AddVideoEncoderConfiguration->ProfileToken));
    char *profile_name_now = azResult[1]; 

    memset(sql, 0, SQL_LENS);
    combin_str(sql, profile_name, 2, "Name", "VideoEncode");
    char *value = find_by_sqlstring(db, sql);
//insert_by_name_cj(db, profile_name_now, 2, "Name", )
    ...
    //        _ns2__AddVideoEncoderConfiguration->ConfigurationToken
